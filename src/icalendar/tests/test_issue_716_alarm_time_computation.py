...
from icalendar.tools import _normalize_pytz
...
def test_can_complete_relative_calculation_if_a_start_is_given(
    ...
    time = alarms.times[0]
    expected_trigger = _normalize_pytz(trigger + alarm_before_start.test_td)
...
def test_can_complete_relative_calculation(alarms, dtend, timezone, trigger, tzp):
    ...
    assert len(alarms.times) == 1
    time = alarms.times[0]
    expected_trigger = _normalize_pytz(trigger - timedelta(days=2))
    assert time.trigger == expected_trigger
