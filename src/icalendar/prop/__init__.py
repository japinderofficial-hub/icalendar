...
from icalendar.tools import _is_date, _is_datetime, _normalize_pytz, _to_datetime
        ...
        if len(ical) == 8:
            if timezone:
                ...
                if tzinfo is not None:
                    return _to_datetime(vDate.from_ical(ical)).replace(tzinfo=tzinfo)
    ...
    def from_jcal(cls, jcal_property: list) -> Self:
        ...
        if params.tzid:
            if isinstance(dt, tuple):
                ...
                end = tzp.localize(dt[1], params.tzid) if _is_datetime(dt[1]) else dt[1]
                dt = (start, end)
            else:
...
class vPeriod(TimeBase):
    ...
    def __init__(
        ...
        if by_duration:
            ...
            end = _normalize_pytz(start + duration)
        else:
            end = end_or_duration
            duration = _normalize_pytz(end - start)
        if start > end:
    ...
    def from_jcal(cls, jcal_property: list) -> Self:
        ...
        if tzid:
            ...
            if _is_datetime(end_or_duration):
